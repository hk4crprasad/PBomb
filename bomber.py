...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/hk4crprasad/PBomb/archive/dev.zip"
        dir_name = "PBomb-dev"
    else:
        zip_url = "https://github.com/hk4crprasad/PBomb/archive/master.zip"
        dir_name = "PBomb-master"
    ...
    if success:
        mesgdcrt.SuccessMessage("PBomb was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update PBomb.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/hk4crprasad/PBomb.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/hk4crprasad/PBomb.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/hk4crprasad/PBomb/master/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("PBomb is up-to-date")
        mesgdcrt.GeneralMessage("Starting PBomb")


def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/hk4crprasad/PBomb/raw/dev/.notify"
        else:
            url = "https://github.com/hk4crprasad/PBomb/raw/master/.notify"
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("PBomb will work only in Python v3")
    sys.exit()
...
description = """PBomb - Your Friendly Spammer Application

PBomb can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

PBomb is not intented for malicious uses.
"""
...
parser.add_argument("-call", "--call", action="store_true",
                    help="start PBomb with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start PBomb with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update PBomb")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current PBomb contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current PBomb version")
